...
:::important
When `customHandle` is enabled, you **must** include a `Sortable.Handle` component within each flex item, otherwise items will not be draggable. See the [Custom Handle](.) documentation page for more details.
:::
