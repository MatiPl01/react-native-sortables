...
This guide describes advanced customization. Familiarity with [react-native-worklets](https://docs.swmansion.com/react-native-reanimated/docs/guides/worklets/) is recommended as strategies run on the UI thread.
...
const useSimpleGridStrategy = () => {
  ...
  const { numGroups: columns } = useGridLayoutContext();
...
return ({ activeIndex, position }) => {
  ...
  // 2. Calculate the target index based on position
...
Here is the complete code for a component that implements and uses a custom strategy. You can copy this entire block into a single file to try it out.
...
// 2. Define the Strategy Factory
const useSimpleSwapStrategy: SortStrategyFactory = () => {
  const { indexToKey } = useCommonValuesContext();
  const { mainGroupSize: itemWidth, numGroups: columns } =
    useGridLayoutContext();

  return ({ activeIndex, position }) => {
    'worklet';

    const col = Math.floor(position.x / ((itemWidth?.value ?? 0) + GAP));
    const row = Math.floor(position.y / (ITEM_HEIGHT + GAP));
    ...
    if (
      ...
      [newOrder[activeIndex], newOrder[targetIndex]] = [
