...
where `SortableGesture` is an opaque gesture object created by the library for the installed `react-native-gesture-handler` version. Pass it to `Sortable.Handle` or another gesture detector without inspecting it.

`ReadonlySharedValue` has the same type as `DerivedValue` from `react-native-reanimated` and works in a similar way to the `SharedValue` type but it doesn't allow setting the value.
