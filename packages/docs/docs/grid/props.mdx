...
import allVideo from '@site/static/video/layout-transition-mode-all.mp4';

import reorderVideo from '@site/static/video/layout-transition-mode-reorder.mp4';
...
:::important
When `customHandle` is enabled, you **must** include a `Sortable.Handle` component within each grid item, otherwise items will not be draggable. See the [Custom Handle](.) documentation page for more details.
:::
